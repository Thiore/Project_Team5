...
public class BlackConnection : MonoBehaviour, ITouchable
{
    ...
    public void OnTouchStarted(Vector2 position)
    {
        ...
        // 현재 오브젝트의 태그가 "BlackConnection"인 경우에만 실행
        if (CompareTag("BlackConnection"))
        {
            // Ray가 충돌한 오브젝트가 있고, 태그가 "Red"인 경우 해당 오브젝트 활성화
            if (hit.collider != null && hit.collider.CompareTag("Red"))
            {
            ...
            // LineRenderer 시작 위치를 현재 오브젝트 위치로 설정
    ...
    public void OnTouchHold(Vector2 position)
    {
        ...
        if (drag_state)
        {
            ...
            // Ray가 "RedConnection" 태그를 가진 오브젝트와 충돌하면
            if (hit.collider.CompareTag("RedConnection"))
            {
                // 자식 오브젝트 비활성화
    ...
    public void OnTouchEnd(Vector2 position)
    {
        ...
        if (drag_state)
        {
            // Ray가 "Red" 태그를 가진 오브젝트와 충돌했으면
            if (hit.collider.CompareTag("Red"))
            {
                Debug.Log("Black 연결 완료!"); // 연결 완료 로그 출력
